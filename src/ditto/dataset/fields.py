...
from src.ditto.dataset import binvox_rw
from src.ditto.dataset.common import coord2index, normalize_coord
from src.ditto.dataset.core import Field
...
class PatchPointsField(Field):
    ...
    def load(self, model_path, idx, vol):
        ...
        ind_list = []
        for i in range(3):
            ind_list.append((points[:, i] >= vol["query_vol"][0][i]) & (points[:, i] <= vol["query_vol"][1][i]))
...
class PatchPointCloudField(Field):
    ...
    def load(self, model_path, idx, vol):
        ...
        ind_list = []
        for i in range(3):
            ind_list.append((points[:, i] >= vol["input_vol"][0][i]) & (points[:, i] <= vol["input_vol"][1][i]))
