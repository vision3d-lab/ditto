...
from src.ditto.dataset.common import decide_total_volume_range, update_reso
...
class Shapes3dDataset(data.Dataset):
    ...
    def __init__(self, dataset_folder, fields, split=None, categories=None, no_except=True, transform=None, cfg=None):
        ...
        if self.cfg["data"]["input_type"] == "pointcloud_crop":
            ...
            recep_field = 128  # 2**(cfg['model']['encoder_kwargs']['unet3d_kwargs']['num_levels'] + 2) # hard-code
            ...
            if self.cfg["generation"]["sliding_window"]:
                self.total_input_vol, self.total_query_vol, self.total_reso = decide_total_volume_range(
                    100000, recep_field, unit_size, depth
                )  # contain the whole scene
            else:
                self.total_input_vol, self.total_query_vol, self.total_reso = decide_total_volume_range(
                    query_vol_metric, recep_field, unit_size, depth
                )
        # print('====len', len(self.models))
    ...
    def get_vol_info(self, model_path):
        ...
        recep_field = 128  # 2**(self.cfg['model']['encoder_kwargs']['unet3d_kwargs']['num_levels'] + 2) # hard-code
        ...
        vol_info = {"plane_type": plane_type, "reso": reso, "input_vol": input_vol, "query_vol": query_vol}
        return vol_info
