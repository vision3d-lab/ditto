...
class MeshIntersector:
    def __init__(self, mesh, resolution=512):
        ...
        # triangles = np.asarray(mesh.vertices)[np.asarray(mesh.triangles)].astype(np.float64) # for cuda mesh
    ...
    def rescale(self, array):
        array = self.scale[None] * array + self.translate[None]
        return array
...
class TriangleIntersector2d:
    ...
    def check_triangles(self, points, triangles):
        ...
        sum_uv = u + v
        contains[mask] = (0 < u) & (u < abs_detA) & (0 < v) & (v < abs_detA) & (0 < sum_uv) & (sum_uv < abs_detA)
        return contains
